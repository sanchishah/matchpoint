...
function SignupForm() {
  ...
  // Validate the referral code on mount and show the referrer's first name
  useEffect(() => {
    ...
    <Card className="border-[#E2E8F0] shadow-none">
      {referralCode && referrerName && (
        <div className="mx-8 mt-8 flex items-center gap-2 rounded-lg bg-[#E8F4F8] px-4 py-3">
          <Gift className="h-4 w-4 text-[#0B4F6C] shrink-0" />
    ...
    <Suspense
      fallback={
        <Card className="border-[#E2E8F0] shadow-none">
          ...
          <CardContent className="px-8 pt-4 flex items-center justify-center min-h-[300px]">
            <Loader2 className="h-6 w-6 animate-spin text-[#0B4F6C]" />
          </CardContent>
        </Card>
      }
    >
